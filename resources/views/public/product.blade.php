    ...
    <div class="container-site grid gap-10 lg:grid-cols-2">
        ...
        <div>
            ...
            <form data-product-builder data-product-id="{{ $product->id }}" data-product-name="{{ $product->name }}" data-product-image="{{ $product->image_url }}" data-price="{{ $product->price }}" class="mt-8 space-y-7">
                @if($product->variants->isNotEmpty())<fieldset>
                    <legend class="mb-3 text-sm font-black">Pilih variasi</legend>
                    <div class="grid gap-3 sm:grid-cols-2">@foreach($product->variants as $variant)<label class="cursor-pointer rounded-2xl border border-stone-200 p-4 has-[:checked]:border-ube-600 has-[:checked]:bg-ube-50"><input type="radio" name="variant_id" value="{{ $variant->id }}" data-name="{{ $variant->variant_name }}" data-price="{{ $variant->additional_price }}" class="mr-2 accent-ube-700" @checked($loop->first)><strong>{{ $variant->variant_name }}</strong><span class="float-right text-sm text-ube-700">+Rp {{ number_format($variant->additional_price,0,',','.') }}</span></label>@endforeach</div>
                </fieldset>@endif
                @if($product->toppings->isNotEmpty())<fieldset>
                    <legend class="mb-3 text-sm font-black">Tambah topping <span class="font-normal text-stone-400">(boleh lebih dari satu)</span></legend>
                    <div class="grid grid-cols-2 gap-3 sm:grid-cols-3">@foreach($product->toppings as $topping)<label class="cursor-pointer overflow-hidden rounded-2xl border border-stone-200 has-[:checked]:border-ube-600 has-[:checked]:ring-2 has-[:checked]:ring-ube-100"><img src="{{ $topping->image_url }}" class="aspect-square w-full object-cover" alt="{{ $topping->name }}"><span class="block p-3"><input type="checkbox" name="topping_ids[]" value="{{ $topping->id }}" data-name="{{ $topping->name }}" data-price="{{ $topping->additional_price }}" class="mr-1 accent-ube-700"><span class="text-xs font-bold">{{ $topping->name }}</span><small class="mt-1 block text-ube-700">+Rp {{ number_format($topping->additional_price,0,',','.') }}</small></span></label>@endforeach</div>
                </fieldset>@endif
                <div class="grid gap-4 sm:grid-cols-[120px_1fr]">
                    <div><label class="label">Jumlah</label><input class="field" type="number" name="quantity" min="1" max="99" value="1"></div>
                    <div><label class="label">Catatan item</label><input class="field" name="notes" maxlength="500" placeholder="Contoh: saus dipisah"></div>
                </div>
                <div class="card flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <p class="text-xs text-stone-400">Total item</p><strong data-live-price class="text-2xl font-black text-ube-800"></strong>
                    </div><button class="btn-primary" type="submit">+ Tambah ke Keranjang</button>
                </div>
