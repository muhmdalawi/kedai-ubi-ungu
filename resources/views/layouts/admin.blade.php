        ...
        <div class="min-w-0">
            <header class="flex items-center justify-between gap-4 border-b border-ube-100 bg-white px-5 py-4 shadow-sm sm:px-8">
                <div class="flex min-w-0 items-center gap-3">
                    ...
                    <div class="min-w-0">
                        <p class="text-xs font-extrabold uppercase tracking-widest text-ube-700">Admin Panel</p>
                        <h1 class="truncate text-xl font-black text-stone-950">@yield('heading','Dashboard')</h1>
                    </div>
                </div>
                <div class="text-right text-sm"><strong class="text-stone-900">{{ auth()->user()->name }}</strong>
                    <p class="hidden text-xs font-medium text-stone-500 sm:block">{{ auth()->user()->email }}</p>
                </div>
            </header>
